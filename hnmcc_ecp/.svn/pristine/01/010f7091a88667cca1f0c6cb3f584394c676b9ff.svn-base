...
public class QueryJfRecommend3Invocation extends BaseInvocation implements
		ILogicalService {
	private static final Logger logger = LoggerFactory.getLogger(QueryJfRecommend3Invocation.class);
	...
	private String REQUEST_BOSSTELETEXT = "qry_jfrecommend_03";

	public QueryJfRecommend3Invocation() {
	...
	public BaseServiceInvocationResult executeService(String accessId,
		...
		QRY060003Result result = new QRY060003Result();
		...
		try {
			...
			List<RecommendDetail3> monthFluxsList = new ArrayList<RecommendDetail3>();
			...
			if(null != jsonObj){
				...
				if(null != body){
					...
					if(null != o){
						...
						RecommendDetail3 recommendDetail;
						com.alibaba.fastjson.JSONObject messageObj;
						for (int i = 0; i < array.size(); i++) {
							messageObj = (com.alibaba.fastjson.JSONObject) array.get(i);
							recommendDetail = new RecommendDetail3();
							recommendDetail.setCampId(messageObj.getString("CampId"));
							recommendDetail.setCampSn(messageObj.getString("CampSn"));
							...
							recommendDetail.setProductCode(messageObj.getString("ProductCode"));
							recommendDetail.setProductName(messageObj.getString("ProductName"));
							recommendDetail.setPromptId(messageObj.getString("PromptId"));
							recommendDetail.setPromptSn(messageObj.getString("PromptSn"));
			...
			logger.info("*******************长度："+monthFluxsList.size());
			logger.info("*******************PkgInfoList："+result.getPkgInfoList());
//			return result;
		}catch (Exception e)
		{
			logger.error("", e);
