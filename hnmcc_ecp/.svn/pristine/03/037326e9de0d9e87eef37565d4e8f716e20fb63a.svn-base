 ...
 * @author wuzd
 * @version jsonVerson
 * @since 2017-03-22 20:40:07
 * @author SongWantao
 *         修改内容，修改logger为动态获取class,增加bossTeletextUtil,remote,wellFormedDAO,ParseXmlConfig的get方法
 */
public class BaseInvocation {
	...
	public ParseXmlConfig getConfig() {
		return config;
	}
	 ...
	 * @throws Exception
	 */
	protected String sendReqXML2BOSS(String accessId, List<RequestParameter> params, String reqXml, String bossTempate)
			throws CommunicateException, Exception {
		return (String) this.remote
				.callRemote(new StringTeletext(reqXml, accessId, bossTempate, this.generateCity(params)));
	}
	 ...
	 * @param resp_desc
	 */
	protected void setCommonResult(BaseServiceInvocationResult res, String resp_code, String resp_desc) {
	...
	protected String genSMSContent(String formatExpectTime, String fmtexpectPeriod, String orderId,
			String marketOrBusiName, String officeName, int optType) {
		String smsContent = "";
		// 预约办理
		if (optType == 1) {
			smsContent = "您已成功预约办理" + marketOrBusiName + "，" + "预约号" + orderId + "，请于" + formatExpectTime
					+ fmtexpectPeriod + "前往" + officeName + "" + "办理！";

		}
		// 预约修改
		if (optType == 2) {
			smsContent = "您已重新预约办理" + marketOrBusiName + "，" + "预约号" + orderId + "，请于" + formatExpectTime
					+ fmtexpectPeriod + "前往" + officeName + "" + "办理！";

		}
		// 预约取消
		if (optType == 3) {
			smsContent = "您已成功取消预约办理" + marketOrBusiName + "，" + "欢迎您再次办理本活动或其他优惠活动！";
