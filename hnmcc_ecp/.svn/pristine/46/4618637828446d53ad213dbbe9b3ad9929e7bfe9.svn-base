...
import com.xwtech.xwecp.service.logic.client_impl.common.ICheckUserService;
import com.xwtech.xwecp.service.logic.pojo.QRY050025Result;

public class CheckUserServiceClientImpl extends BaseClientServiceImpl implements ICheckUserService
{
	public QRY050025Result checkUser(String phoneNum, String proId, String brandId, int oldPayMode, int payMode) throws LIException
	{
		...
		RequestParameter __param_oldPayMode = new RequestParameter("oldPayMode", oldPayMode);
		__requestData.getParams().add(__param_oldPayMode);
		RequestParameter __param_payMode = new RequestParameter("payMode", payMode);
		__requestData.getParams().add(__param_payMode);
		...
		try
		{
			...
			if(__ret instanceof QRY050025Result)
			{
				__result = (QRY050025Result)__ret;
			}
			else
			{
				__result = new QRY050025Result();
