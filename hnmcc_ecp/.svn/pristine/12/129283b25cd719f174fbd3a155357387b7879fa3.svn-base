...
public class CheckUserInfoServiceClientImpl extends BaseClientServiceImpl implements ICheckUserInfoService
{
	public QRY040005Result checkUserInfo(String phoneNum, String icType, String icNo, int type) throws LIException
	{
		long __beginTime = System.currentTimeMillis();
		String __cmd = "QRY040005";
		...
		RequestParameter __param_icType = new RequestParameter("icType", icType);
		__requestData.getParams().add(__param_icType);
		...
		try
		{
			...
			if(__ret instanceof QRY040005Result)
			{
				__result = (QRY040005Result)__ret;
			}
			else
			{
				__result = new QRY040005Result();
