...
/**
 * 根据meta文件设置的参数 解析实时账单查询结果
 * 
 * @author 丁亮 Apr 19, 2011
 */
public class QueryNewRealTimeBillingInvocationByJSON extends BaseInvocation implements ILogicalService {
	...
	// private String REQUEST_BOSSTELETEXT_920131 =
	// "ac_acqryrealtimebill_920131";
	...
	public BaseServiceInvocationResult executeService(String accessId, ServiceConfig config,
		...
		try {
			...
			String rspXml = util.getResponseXML(accessId, config, params);
			util.distroy();
	...
	public QRY010018Result getCollectionCharges(String accessId, ServiceConfig config, List<RequestParameter> params) {
		...
		try {
			BaseJsonQueryUtils util = new BaseJsonQueryUtils();
			util.setConfig(REQUEST_BOSSTELETEXT_920138);
			String rspXml = util.getResponseXML(accessId, config, params);
			util.distroy();
	...
	public QRY010018Result getFeeDetail(String accessId, ServiceConfig config, List<RequestParameter> params) {
		...
		try {
			BaseJsonQueryUtils util = new BaseJsonQueryUtils();
			util.setConfig(REQUEST_BOSSTELETEXT_920131);
			String rspXml = util.getResponseXML(accessId, config, params);
			util.distroy();
