...
public class DefaultServiceImpl extends BaseServerServiceImpl implements IService {
	...
	static {
		cmds.append("QRY030011").append("QRY040020").append("QRY040041").append("QRY040048").append("QRY040065")
				.append("QRY040070");
		...
		if (teletext != null) {
			...
			try {
				...
				bossResponse = (String) this.remote.callRemote(
						new StringTeletext(teletext, accessId, tm.getTeletextTemplate(), remoteCallContext));
	...
	private void evaluateResult2Array(String cmd, String bossFunctionId, String bossResponse,
		...
		this.evaluateResult2Array(null, result, resultMapping, objStack);
	}
	 ...
	 * @throws Exception
	 */
	private void evaluateResult(String cmd, String bossFunctionId, String bossResponse,
								BaseServiceInvocationResult result, String bossTemplateId) throws Exception {
		PerformanceTracer pt = PerformanceTracer.getInstance();
		long n = pt.addBreakPoint();
	 ...
	 * @throws Exception
	 */
	private void evaluateResult(Object item, Object obj, List<ServiceResultMapping> resultMapping,
	...
	private void evaluateResult2Array(Object item, Object obj, List<ServiceResultMapping> resultMapping,
		...
		for (int i = 0; i < resultMapping.size(); i++) {
			...
			// TODO 取CRM返回字段的下标
			String index = resultMap.getIndex();
			...
			if (resultMap.getChildResultMapping() != null && resultMap.getChildResultMapping().size() > 0) {
				...
				if (DataTypeConstants.LIST.equalsIgnoreCase(outputField.getDataType())) {
					...
					// this.evaluateResult(newItem, newObject,
					// resultMap.getChildResultMapping(), objStack);
					callSetter(obj, resultMap.getName(), newObject, DataTypeConstants.CLASS);
				}
			} else {
				String value = "";
	...
	private TeletextMapping getBossTeletextMapping() {
		...
		for (int i = 0; i < this.serviceConfig.getImpl().getTeletextMapping().size(); i++) {
			...
			// if ("$sys_channel_param".equals(paramName) &&
			// channel.equals(match)) {
			//
			// return m;
			// }
			/**
	...
	private ServiceOutputField getOutputFieldInfo(String name) {
		for (int i = 0; i < this.getServiceConfig().getOutput().getOutputFields().size(); i++) {
			if (name.equals(this.getServiceConfig().getOutput().getOutputFields().get(i).getName())) {
				return this.getServiceConfig().getOutput().getOutputFields().get(i);
			}
		}
		return null;
	}
	...
	public static class StringTeletext implements IStreamableMessage {
		/* jboss remoting 参数添加 2016年11月17日 16:32:40 start */
		/** 默认类型 0 */
		public final static String DEFAULT_REQ_TYPE = "0";
		/** jboss remoting 请求类型 1 */
		public final static String REMOTING_REQ_TYPE = "1";

		protected String sysParam;
		protected String busiParam;
		/**
		 * 模版使用类型 默认为0,1使用json字符串模版
		 */
		...
		protected String jfserialNum;
		protected String prodId;

		/**
		 * 2016年11月17日 16:34:59 添加新的构造方法
		 *
		 * @param sysParam
		 * @param busiParam
		 * @param type
		 ...
		 * @param accessId
		 * @param identity
		 * @param remoteCallContext
		 */
		public StringTeletext(String sysParam, String busiParam, String type, String teletext, String accessId,
							  String identity, RemoteCallContext remoteCallContext, String traceId, String userMobile,
							  String userBrand, String userCity,String pageNum,String recNum,String serialNum,String jfserialNum) {
			super();
		...
		public StringTeletext(String sysParam, String busiParam, String type, String teletext, String accessId,
							  String identity, RemoteCallContext remoteCallContext, String traceId, String userMobile,
							  String userBrand, String userCity,String pageNum,String recNum,String serialNum,String jfserialNum,String prodId) {
			super();
			this.sysParam = sysParam;
			this.busiParam = busiParam;
			this.type = type;
		...
		/* jboss remoting 参数添加 2016年11月17日 16:32:40 end */
