...
import com.xwtech.xwecp.flow.works.chains.nodes.*;
...
public class WorkFlowFacade
{
	...
	private WorkFlowFacade()
	{
		// 建立节点流程
		// 1. 白名单验证节点
		startFlowControl = new CheckWhiteNode();
		// 1. 接入权限验证节点
		IFlowControl flowControl = startFlowControl.setNext(new CheckAccessPrivilegeNode());
		// 2. 设置 流量控制验证节点
		flowControl = flowControl.setNext(new CheckChannelFlowNode());
		// 3. 设置接口调用权限验证节点
