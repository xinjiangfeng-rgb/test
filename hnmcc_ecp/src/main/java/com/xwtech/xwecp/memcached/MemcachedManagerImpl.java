...
//			.getLogger(MemcachedManagerImpl.class);
//
...
//		memCachedClient = new MemCachedClient(serverName);
//		// 开始验证...
//		// 缓存服务器和其权重是否配置验证
...
//		if (initConn != null
//				&& !Pattern.compile("[0-9]*").matcher(initConn).matches()) {
//			logger.error("初始连接数配置不能为字符......加载失败！");
//		}
//		if (minConn != null
//				&& !Pattern.compile("[0-9]*").matcher(minConn).matches()) {
//			logger.error("最小连接数配置不能为字符......加载失败！");
//		}
//		if (maxConn != null
//				&& !Pattern.compile("[0-9]*").matcher(maxConn).matches()) {
//			logger.error("最大连接数配置不能为字符......加载失败！");
//		}
//		if (maxIdle != null
//				&& !Pattern.compile("[0-9]*").matcher(maxIdle).matches()) {
//			logger.error("最大处理时间配置不能为字符......加载失败！");
//		}
//
//		// 主线程的睡眠时间验证
//		if (maintSleep != null
//				&& !Pattern.compile("[0-9]*").matcher(maintSleep).matches()) {
//			logger.error("主线程睡眠时间配置不能为字符......加载失败！");
//		}
//
//		// TCP的参数，连接超时验证
//		if (nagle != null && !nagle.trim().equals("")
//				&& !nagle.trim().equals("false")
//				&& !nagle.trim().equals("true")) {
//			logger.error("TCP参数nagle配置只能为空值或者true或者false......加载失败！");
//		}
//		if (socketTo != null
//				&& !Pattern.compile("[0-9]*").matcher(socketTo).matches()) {
//			logger.error("TCP参数socketTo配置不能为字符......加载失败！");
//		}
//		if (socketConnectTo != null
//				&& !Pattern.compile("[0-9]*").matcher(socketConnectTo)
//						.matches()) {
...
//		// 验证结束...
//
//		System.out.println("IP:" + servers[0]);
//		// 获取socke连接池的实例对象
//		SockIOPool pool = SockIOPool.getInstance(this.serverName);
//
//		// 设置服务器信息
...
//		if (nagle != null
//				&& !nagle.trim().equals("")
//				&& (nagle.trim().equals("false") || nagle.trim().equals("true"))) {
...
//		Date d = new Date(expireInSeconds);
